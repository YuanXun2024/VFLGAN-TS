...
folder_0_withSD = 'params_vfl/energy_data_24/with_SD/'
folder_0_withoutSD = 'params_vfl/energy_data_24/without_SD/'
folder_0_coscigan= 'params_coscigan/energy_data_24/'
...
for i in range(n_groups_1):
    ...
    params_path = f'params_vfl/energy_data_24/with_SD/Generator_1_channel_{i}_epoch_{epoch}.pt'
...
for i in range(n_groups_2):
    ...
    params_path = f'params_vfl/energy_data_24/with_SD/Generator_2_channel_{i}_epoch_{epoch}.pt'
...
for i in range(n_groups_1):
    generator_1[i] = LSTMGenerator(latent_dim=noise_len, ts_dim=n_samples).to(device)
    params_path = f'params_vfl/energy_data_24/without_SD/Generator_1_channel_{i}_epoch_{epoch}.pt'
    generator_1[i].load_state_dict(torch.load(params_path))
    generator_1[i].eval()

for i in range(n_groups_2):
    generator_2[i] = LSTMGenerator(latent_dim=noise_len, ts_dim=n_samples).to(device)
    params_path = f'params_vfl/energy_data_24/without_SD/Generator_2_channel_{i}_epoch_{epoch}.pt'
    generator_2[i].load_state_dict(torch.load(params_path))
    generator_2[i].eval()
...
for i in range(n_groups):
    ...
    params_path = f'params_coscigan/energy_data_24/Generator_1_channel_{i}_epoch_{epoch}.pt'
