...
parser.add_argument('--dataset', type=str, default='data_frame_sine_normal_6')
...
def main():
    ...
    for i in range(n_groups):
        if i==0:
            real_sin = fit_sin(train[:, i*n_samples:(i+1)*n_samples].detach(), n_samples).reshape(-1,1)
        else:
            real_sin_tmp = fit_sin(train[:, i*n_samples:(i+1)*n_samples].detach(), n_samples).reshape(-1,1)
            real_sin = np.concatenate((real_sin, real_sin_tmp), axis=1)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 5 == 0:
            ...
            for i in range(n_groups_1):
                ...
                results_sin = fit_sin(generated_samples_1[i].detach().cpu(), n_samples)
                if i==0:
                    results_all = results_sin.reshape(-1,1)
                else:
                    results_all = np.concatenate((results_all, results_sin.reshape(-1,1)), axis=1)
            for i in range(n_groups_2):
                generated_samples_2[i] = generators_2[i](new_noise).float()
                results_sin = fit_sin(generated_samples_2[i].detach().cpu(), n_samples)
                results_all = np.concatenate((results_all, results_sin.reshape(-1,1)), axis=1)

            idxs_1 = np.random.choice(a=1024, size=256, replace=False, p=None)
            idxs_2 = np.random.choice(a=1024, size=256, replace=False, p=None)

            wd_all = wd_nd(results_all[idxs_1, :], real_sin[idxs_2, :])
