...
def compute_acc(scores_shadow, scores_loo):
    ...
    thres_min = min(scores_shadow+scores_loo)
    thres_max = max(scores_shadow+scores_loo)
    ...
    for i in range(gap):
        thres_temp = thres_min + i * (thres_max-thres_min) / gap
        acc_tmp = (len(np.where(train_s<=thres_temp)[0]) + len(np.where(train_l>thres_temp)[0])) / (len(train_s)+len(train_l))
        if acc_tmp > max_train_acc:
            max_train_acc = acc_tmp
            thres = thres_temp
...
if __name__=='__main__':
    LOOs = [704, 1129]   # 688: proposed method; 396: proposed v4; 704: Record 1 (groundhog day); 1129: Record 2.

    # LOOs = [688]
    LOOs = [730]

    df = pd.read_csv('../Dataset/stock_data_24.csv')
    num = df.shape[0]
