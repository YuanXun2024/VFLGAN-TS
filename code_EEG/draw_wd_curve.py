...
def calculateWD_vfl(root='params_vfl'):
    datasets = os.listdir(root)

    for dataset in datasets:
        data = pd.read_csv(f'../Dataset/{dataset}.csv')
        real_samples = data.values

        path_dataset = os.path.join(root, dataset)
...
def calculateWD_coscigan(root='params_coscigan'):
    datasets = os.listdir(root)
